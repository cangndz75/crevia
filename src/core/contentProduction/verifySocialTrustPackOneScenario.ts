...
import {
  ...
  type SocialTrustPackOneDomain,
  type SocialTrustPackOneEchoSurface,
  type SocialTrustPackOneFamily,
  type SocialTrustPackOneVariantKind,
} from './contentPacks';
...
const CUMHURIYET_MIN = 4;
const MERKEZ_MIN = 4;
const YESILVADI_MIN = 3;
const ISTASYON_MIN = 3;
const SANAYI_MIN = 2;

const REQUIRED_VARIANTS: readonly SocialTrustPackOneVariantKind[] = [
  'normal',
  'improved',
  'worsened',
  'carry_over',
...
const REQUIRED_DOMAINS: readonly SocialTrustPackOneDomain[] = [
  'social_trust',
  'public_sentiment',
  'district_trust',
  'district_memory',
...
const REQUIRED_ECHO_SURFACES: readonly SocialTrustPackOneEchoSurface[] = [
  'advisor',
  'report',
  'social',
  'map',
...
const SOCIAL_ECHO_FORBIDDEN_TERMS = ['siyasi parti', 'belediye baskan', 'hakaret', 'gercek kisi adi'] as const;
...
export type VerifySocialTrustPackOneOutcome = {
  ok: boolean;
  warn: boolean;
  checks: string[];
};
...
function familyText(family: SocialTrustPackOneFamily): string {
  return normalizeText(
    [
      ...
      family.socialTrustIntent,
      family.publicSentimentIntent,
      family.districtTrustIntent,
      family.districtMemoryIntent,
      family.socialMentionIntent,
      ...
      ...Object.values(family.socialHints),
    ].join(' '),
  );
}
...
export function verifySocialTrustPackOneScenario(): VerifySocialTrustPackOneOutcome {
  const checks: string[] = [];
  let ok = true;
  let warnFlag = false;
  ...
  const totalVariants = SOCIAL_TRUST_PACK_ONE_FAMILIES.reduce(
  ...
  record(
    assert(
      checks,
      SOCIAL_TRUST_PACK_ONE_CONTENT_PACK.id === SOCIAL_TRUST_PACK_ONE_ID,
      'pack id',
      'pack id mismatch',
    ),
  );
  record(
    assert(
      checks,
      SOCIAL_TRUST_PACK_ONE_CONTENT_PACK.kind === 'event_family_pack',
  ...
  record(
    assert(
      checks,
      SOCIAL_TRUST_PACK_ONE_CONTENT_PACK.isRuntimeLinked === false,
      'pack is not runtime linked',
      'pack runtime linked',
    ),
  );
  record(assert(checks, SOCIAL_TRUST_PACK_ONE_FAMILIES.length >= 16, '16+ event families', 'family count below 16'));
  record(
    assert(
      checks,
      SOCIAL_TRUST_PACK_ONE_CONTENT_PACK.items.length === SOCIAL_TRUST_PACK_ONE_FAMILIES.length,
  ...
  const familiesByDistrict = getSocialTrustPackOneFamiliesByDistrict();
  record(
    assert(
      checks,
  ...
  for (const family of SOCIAL_TRUST_PACK_ONE_FAMILIES) {
    ...
    record(
      assert(
        checks,
        family.socialTrustIntent.length > 0 || family.publicSentimentIntent.length > 0,
        `${family.id} social trust or public sentiment intent`,
        `${family.id} social/public intent missing`,
      ),
    );
    record(
      assert(
    ...
    record(assert(checks, family.socialMentionIntent.length > 0, `${family.id} social mention intent`, `${family.id} mention intent missing`));
    record(assert(checks, family.socialTrustIntent.length > 0, `${family.id} social trust intent`, `${family.id} social trust missing`));
    record(assert(checks, family.publicSentimentIntent.length > 0, `${family.id} public sentiment intent`, `${family.id} public sentiment missing`));
    ...
    record(assert(checks, family.socialHints.socialMentionHint.length > 0, `${family.id} social mention hint`, `${family.id} mention hint missing`));
    record(assert(checks, family.socialHints.districtTrustHint.length > 0, `${family.id} district trust hint`, `${family.id} trust hint missing`));
    record(assert(checks, family.socialHints.advisorToneHint.length > 0, `${family.id} advisor tone hint`, `${family.id} advisor hint missing`));
    record(assert(checks, family.socialHints.reportPublicReactionHint.length > 0, `${family.id} report public reaction hint`, `${family.id} report hint missing`));
    ...
    const socialEcho = family.echoes.social;
    const advisorEcho = family.echoes.advisor;
    record(
      assert(
        checks,
        socialEcho.length >= advisorEcho.length || socialEcho.includes(':'),
        `${family.id} social echo livelier than advisor`,
        `${family.id} social echo flat`,
      ),
    );
    const socialEchoNorm = normalizeText(socialEcho);
    for (const term of SOCIAL_ECHO_FORBIDDEN_TERMS) {
      record(
        assert(
          checks,
          !socialEchoNorm.includes(normalizeText(term)),
          `${family.id} social echo avoids ${term}`,
          `${family.id} social echo contains ${term}`,
        ),
      );
    }
  ...
  const variantCoverage = getSocialTrustPackOneVariantCoverage();
  ...
  const echoCoverage = getSocialTrustPackOneEchoSurfaceCoverage();
  for (const surface of REQUIRED_ECHO_SURFACES) {
    record(
      assert(
        checks,
        echoCoverage[surface] === SOCIAL_TRUST_PACK_ONE_FAMILIES.length,
  ...
  for (const domain of REQUIRED_DOMAINS) {
    const covered =
      SOCIAL_TRUST_PACK_ONE_FAMILIES.some((family) => family.domains.includes(domain)) ||
      SOCIAL_TRUST_PACK_ONE_CONTENT_PACK.items.some(
  ...
  const validation = validateContentPackDefinition(SOCIAL_TRUST_PACK_ONE_CONTENT_PACK);
  record(assert(checks, validation.status !== 'fail', 'pack validation not fail', `pack validation ${validation.status}`));

  const echoCompleteness = evaluateEchoCompletenessForPack(SOCIAL_TRUST_PACK_ONE_CONTENT_PACK);
  record(
    assert(
      checks,
  ...
  for (const item of SOCIAL_TRUST_PACK_ONE_CONTENT_PACK.items) {
    for (const surface of SOCIAL_TRUST_PACK_ONE_REQUIRED_ECHO_SURFACES) {
      record(
        assert(
          checks,
  ...
  const internalDuplicates = findContentDuplicateRisks(SOCIAL_TRUST_PACK_ONE_ITEMS);
  record(
    assert(
      checks,
  ...
  for (const packItem of SOCIAL_TRUST_PACK_ONE_ITEMS) {
  ...
  record(
    assert(
      ...
      'no cross-pack collision with district vehicle and container packs',
      `cross-pack collision pairs ${crossPackCollision}`,
    ),
  );

  const audit = buildContentProductionAuditResult([SOCIAL_TRUST_PACK_ONE_CONTENT_PACK]);
  ...
  const coverage = buildContentCoverageMatrix([SOCIAL_TRUST_PACK_ONE_CONTENT_PACK]);
  ...
  const playerCopy = normalizeText(
    collectContentProductionPlayerFacingCopy([SOCIAL_TRUST_PACK_ONE_CONTENT_PACK]).join(' '),
  );
  for (const term of EXTRA_FORBIDDEN_TERMS) {
  ...
  const docs = readRepo('docs/crevia-social-trust-pack-one.md').toLowerCase();
  ...
  record(recordScenario(checks, verifyDynamicSocialEchoScenario(), 'dynamic social echo'));
  record(recordScenario(checks, verifySocialPulseUiScenario(), 'social pulse ui'));
  ...
  for (const surface of targetSurfaces) {
    record(
      assert(
        checks,
        SOCIAL_TRUST_PACK_ONE_CONTENT_PACK.targetSurfaces.includes(surface),
