...
export function verifyStoryChainPersistentRuntimeReviewScenario(): VerifyStoryChainPersistentRuntimeReviewOutcome {
  ...
  ok =
    assert(
      checks,
      !result.saveImpact.saveVersionChanged && SAVE_VERSION === 23,
  ...
  ok = assert(checks, SAVE_VERSION === 23, 'SAVE_VERSION 23 unchanged (cross-verify)', `SAVE_VERSION=${SAVE_VERSION}`) && ok;
